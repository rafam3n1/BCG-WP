(function ($) {
    ...
    const POPUP_ESCOLHA_MAQUINA = 109347;
    const CONFIG_BUTTONS = [
        { id: "btn-gtx1060", key: "GTX 1060|Intel Xeon E5-2640 v3|16GB" },
        { id: "btn-rtx2060s", key: "RTX 2060 SUPER|Intel Xeon E5-2640 v3|16GB" },
        { id: "btn-rx7600", key: "RX 7600|AMD Ryzen 7 5700|16GB" },
        { id: "btn-rx6650-16", key: "RX 6650 XT|AMD Ryzen 7 5700|16GB" },
        { id: "btn-rx6650-12", key: "RX 6650 XT|AMD Ryzen 7 5700|12GB" },
    ];

    function getDeviceId() {
        const storageKey = 'BCG_DEVICE_ID';
    ...
    socket.on("escolherMaquina", function (msg) {
        try {
            var configs = (msg && msg.configs) || [];
            if (!POPUP_ESCOLHA_MAQUINA) {
                console.warn("[BCG] POPUP_ESCOLHA_MAQUINA não configurado — defina o ID do popup Elementor.");
                return;
            }
            ...
            function aplicar() {
                CONFIG_BUTTONS.forEach(function (b) {
                    var el = document.getElementById(b.id);
                    if (!el) return;
                    var cfg = byKey[b.key];
                    if (cfg) {
                        el.style.display = "";
                        var cnt = el.querySelector(".bcg-count");
                        if (cnt) cnt.textContent = cfg.count;
                        el.onclick = function (ev) {
                            ev.preventDefault();
                            socket.emit("maquinaEscolhida", { key: b.key });
                            try { elementorProFrontend.modules.popup.hidePopup({ id: POPUP_ESCOLHA_MAQUINA }); } catch (e) { }
                            $("#jogos")[0].style.display = "none";
                            $("#loading")[0].style.display = "flex";
                            var p = document.getElementById("status_text");
                            var f = p.querySelector(".ekit-fancy-prefix-text");
                            f.innerHTML = "Aguarde, sua máquina será iniciada";
                        };
                    } else {
            ...
            aplicar();
            elementorProFrontend.modules.popup.showPopup({ id: POPUP_ESCOLHA_MAQUINA });
            setTimeout(aplicar, 80);
        } catch (e) {
            console.error("[BCG] Erro ao montar popup de seleção de máquina:", e);
        }
    });
    ...
    $(document).ready(function () {
        ...
        $("#wireguard-conf")[0].onclick = function () {
            socket.emit("wireguard", { deviceId: window.deviceId });
        };
